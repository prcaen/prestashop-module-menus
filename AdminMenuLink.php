...
class AdminMenuLink extends AdminTab
{
	...
	protected $identifiersDnd = array('id_menu_link' => 'id_menu_link', 'id_menu' => 'id_menu_to_move');
	...
	public function displayListContent($token = NULL)
	{
		...
		if ($this->_list)
		{
			...
			foreach ($this->_list AS $tr)
			{
				...
				echo '<tr'.(array_key_exists($this->identifier,$this->identifiersDnd) ? ' id="tr_'.(($id_menu = (int)(Tools::getValue('id_menu', '1'))) ? $id_menu : '').'_'.$id.'_'.$tr['position'].'"' : '').($irow++ % 2 ? ' class="alt_row"' : '').' '.((isset($tr['color']) AND $this->colorOnBackground) ? 'style="background-color: '.$tr['color'].'"' : '').'>
							<td class="center">';
				...
				foreach ($this->fieldsDisplay AS $key => $params)
				{
					...
					echo '
					<td '.(isset($params['position']) ? ' id="td_'.(isset($id_menu) AND $id_menu ? $id_menu : 0).'_'.$id.'"' : '').' class="'.((!isset($this->noLink) OR !$this->noLink) ? 'pointer' : '').((isset($params['position']) AND $this->_orderBy == 'position')? /*' dragHandle'*/ '' : ''). (isset($params['align']) ? ' '.$params['align'] : '').'" ';
					...
					if (isset($params['active']) AND isset($tr[$key]))
						$this->_displayEnableLink($token, $id, $tr[$key], $params['active'], Tools::getValue('id_menu'), Tools::getValue('id_menu_link'));
					...
					elseif (isset($params['position']))
					{
						if ($this->_orderBy == 'position' AND $this->_orderWay != 'DESC')
						{
							echo '<a'.(!($tr[$key] != $positions[sizeof($positions) - 1]) ? ' style="display: none;"' : '').' href="'.$currentIndex.
									'&'.$keyToGet.'='.(int)($id_menu).'&'.$this->identifiersDnd[$this->identifier].'='.$id.'
	...
	public function postProcess($token = NULL)
	{
		...
		elseif (Tools::getValue('position'))
		{
			...
			elseif (!$object->updatePosition((int)(Tools::getValue('way')), (int)(Tools::getValue('position'))))
				...
				Tools::redirectAdmin($currentIndex.'&'.$this->table.'Orderby=position&'.$this->table.'Orderway=asc&conf=4&updatemenu'.(($id_menu = (int)(Tools::getValue('id_menu'))) ? ('&id_menu='.$id_menu) : '').'&token='.Tools::getAdminTokenLite('AdminMenus'));
		}
